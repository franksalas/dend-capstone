from clean_helper_functions import *


# clean data
def clean_crime_weather_data():
    # 2009
    raw = bucket_raw_path('dend-data','capstone/raw-data/crime-data/2009')
    d09 = s3_files_to_df(raw)
    df09 = clean_2009(d09)
    save_to_S3_c(df09,'2009')

    # 2010
    raw = bucket_raw_path('dend-data','capstone/raw-data/crime-data/2010')
    c10 = s3_files_to_df(raw)
    df10 = clean_2010(c10)
    save_to_S3_c(df10,'2010')
    #2011
    raw = bucket_raw_path('dend-data','capstone/raw-data/crime-data/2011')
    d11 = s3_files_to_df(raw)
    df11 = clean_2011(d11)
    save_to_S3_c(df11,'2011')

    # 2012
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2012')
    d12 = s3_files_to_df(raw)
    df12 = clean_2012(d12)
    save_to_S3_c(df12,'2012')

    # 2013
    raw = bucket_raw_path('dend-data','capstone/raw-data/crime-data/2013')
    d13 = s3_files_to_df(raw)
    df13 = clean_2013(d13)
    save_to_S3_c(df13,'2013')

    # 2014
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2014')
    d14 = s3_files_to_df_2014(raw)
    df14 = clean_2014(d14)
    save_to_S3_c(df14,'2014')

    # 2015
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2015')
    d15 = s3_files_to_df(raw)
    df15 = clean_2015(d15)
    save_to_S3_c(df15,'2015')

    # 2016
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2016')
    d16 = s3_files_to_df(raw)
    df16 = clean_2016(d16)
    save_to_S3_c(df16,'2016')

    # 2017
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2017')
    d17 = s3_files_to_df(raw)
    df17 = clean_2017(d17)
    save_to_S3_c(df17,'2017')

    # 2018
    raw = bucket_raw_path('dend-data',f'capstone/raw-data/crime-data/2018')
...
def combine_crime_data():
    ...
    raw = bucket_raw_path('dend-data','capstone/inter-data/crime-data/')
...
def crime_weather_merge():
    ...
    wraw = bucket_raw_path('dend-data',f'capstone/inter-data/weather-data/')
